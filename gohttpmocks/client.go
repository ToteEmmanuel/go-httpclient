...
func (c *httpClientMock) Do(request *http.Request) (*http.Response, error) {
	...
	mock := MockedServer.mocks[MockedServer.getMockKey(request.Method, request.URL.String(), string(body))]
	if mock != nil {
		if mock.Error != nil {
	...
	return nil, fmt.Errorf("no mock matching %s from '%s' with given body", request.Method, request.URL)
}
